...
public class NodeListSearch {
	...
	//ノードリストに存在しているノード番号を返す
	/**
	 * @return ノードリストに存在しているノード番号を返す
	 */
	public ArrayList<Integer> ExistNode(){
		ArrayList<Integer> existnodelist = new ArrayList<Integer>();

		for(int i=0; i<nodelist.size(); i++){
				if( existnodelist.contains(nodelist.get(i).getParent_node())){
					continue;
				}
				else{
					existnodelist.add(nodelist.get(i).getParent_node());
				}
				if( existnodelist.contains(nodelist.get(i).getChild_node())){
					continue;
				}
				else{
